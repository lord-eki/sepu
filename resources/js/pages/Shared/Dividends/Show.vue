  ...
  <AppLayout :breadcrumbs="[
    ...
    flashType === 'success'
      ...
      : 'bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 text-red-800 dark:text-red-400'
          ...
          <svg v-if="flashType === 'success'" class="h-5 w-5 text-green-600 dark:text-green-400" viewBox="0 0 20 20"
            fill="currentColor">
            <path
          ...
          <button type="button"
            class="ml-auto text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-white"
    ...
    getStatusClass(dividend.status)
      ...
      <div class="flex flex-wrap items-start gap-3 mt-3 md:mt-0">
        ...
        <button v-if="canDistribute" @click="showDistributeModal = true"
          ...
          <svg class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
            <path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 
    ...
    <main class="max-w-7xl px-4 sm:px-6 lg:px-8 mt-6 space-y-6">
      ...
      <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="rounded-xl shadow-sm bg-white dark:bg-gray-800 p-5 border border-orange-100 dark:border-gray-700">
          ...
          <p class="mt-2 text-xl font-bold text-blue-900 dark:text-gray-100">KSh {{
    formatCurrency(dividend.total_profit) }}</p>
        </div>
        ...
        <div class="rounded-xl shadow-sm bg-white dark:bg-gray-800 p-5 border border-orange-100 dark:border-gray-700">
          ...
          <p class="mt-2 text-xl font-bold text-blue-900 dark:text-gray-100">KSh {{
    formatCurrency(dividend.total_dividends) }}</p>
        </div>
      ...
      <section class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="rounded-xl shadow-md bg-white dark:bg-gray-900 p-6 border border-gray-100 dark:border-gray-700">
          ...
          <div class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
            ...
            <div class="flex justify-between"><span>Members Paid</span><strong
                class="text-green-600 dark:text-green-400">{{ stats.members_paid }}</strong></div>
            <div class="flex justify-between"><span>Members Pending</span><strong
                class="text-orange-500 dark:text-orange-400">{{ stats.members_pending }}</strong></div>
        ...
        <div class="rounded-xl shadow-md bg-white dark:bg-gray-900 p-6 border border-gray-100 dark:border-gray-700">
          ...
          <div class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
            <div class="flex justify-between"><span>Total Distributed</span><strong
                class="text-green-600 dark:text-green-400">KSh {{ formatCurrency(stats.total_paid) }}</strong></div>
            <div class="flex justify-between"><span>Pending Distribution</span><strong
                class="text-orange-500 dark:text-orange-400">KSh {{ formatCurrency(stats.total_pending) }}</strong>
            </div>
            ...
            <div class="border-t pt-3">
              ...
              <div class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full mt-2 overflow-hidden">
                <div class="h-full bg-gray-900 dark:bg-gray-100 rounded-full transition-all duration-500"
      ...
      <!-- Timeline -->
      <section
        class="rounded-xl shadow-md bg-white dark:bg-gray-900 p-6 border border-gray-100 dark:border-gray-700 mt-6">
        ...
        <div class="ml-4 pl-6 border-l-2 border-gray-200 dark:border-gray-700 flex flex-col gap-6">
          ...
          <div class="relative flex items-center gap-4">
            <div
              class="flex-shrink-0 h-8 w-8 rounded-full bg-gray-900 dark:bg-gray-700 flex items-center justify-center text-white shadow-sm">
              C</div>
            <div class="flex-1">
              <p class="text-sm text-gray-600 dark:text-gray-300">
                Calculated on <strong class="text-gray-900 dark:text-gray-100">{{ formatDate(dividend.calculation_date)
                  }}</strong>
                <span v-if="dividend.calculated_by" class="ml-2 text-gray-700 dark:text-gray-400">by {{
    dividend.calculated_by.name }}</span>
              </p>
            </div>
          </div>
          ...
          <div v-if="dividend.approval_date" class="relative flex items-center gap-4">
            <div
              class="flex-shrink-0 h-8 w-8 rounded-full bg-green-600 flex items-center justify-center text-white shadow-sm">
              A</div>
            <div class="flex-1">
              <p class="text-sm text-gray-600 dark:text-gray-300">
                Approved on <strong class="text-gray-900 dark:text-gray-100">{{ formatDate(dividend.approval_date)
                  }}</strong>
                <span v-if="dividend.approved_by" class="ml-2 text-gray-700 dark:text-gray-400">by {{
    dividend.approved_by.name }}</span>
              </p>
              <p v-if="dividend.approval_notes" class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{
    dividend.approval_notes }}</p>
            </div>
          </div>
          ...
          <div v-if="dividend.distribution_date" class="relative flex items-center gap-4">
            <div
              class="flex-shrink-0 h-8 w-8 rounded-full bg-orange-500 flex items-center justify-center text-white shadow-sm">
              D</div>
            <div class="flex-1">
              <p class="text-sm text-gray-600 dark:text-gray-300">
                Distributed on <strong class="text-gray-900 dark:text-gray-100">{{
    formatDate(dividend.distribution_date) }}</strong>
              </p>
            </div>
          </div>
          ...
          <div v-if="dividend.reversed_at" class="relative flex items-center gap-4">
            <div
              class="flex-shrink-0 h-8 w-8 rounded-full bg-red-600 flex items-center justify-center text-white shadow-sm">
              R</div>
            <div class="flex-1">
              <p class="text-sm text-gray-600 dark:text-gray-300">
                Reversed on <strong class="text-gray-900 dark:text-gray-100">{{ formatDate(dividend.reversed_at)
                  }}</strong>
              </p>
      ...
      <section
        class="rounded-xl shadow-md bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 overflow-x-auto mt-6">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <div class="flex items-center gap-3 text-sm">
            <Link :href="route('dividends.members', dividend.id)"
              class="text-gray-900 dark:text-gray-100 hover:underline">View All Members</Link>
            <span class="text-gray-200 dark:text-gray-500">|</span>
            <Link :href="route('dividends.report', dividend.id)"
              class="text-gray-900 dark:text-gray-100 hover:underline">Generate Report</Link>
          </div>
        </div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-700">
            <tr v-for="memberDividend in memberDividends.data" :key="memberDividend.id"
              ...
              <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">KSh {{
    formatCurrency(memberDividend.shares_balance) }}</td>
              <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">KSh {{
    formatCurrency(memberDividend.dividend_amount) }}</td>
              <td class="px-6 py-4">
                <span
              ...
              <td class="px-6 py-4 text-right text-sm">
                <div class="flex justify-end items-center gap-3">
                  <Link :href="route('dividends.member-details', [dividend.id, memberDividend.member.id])"
                    class="text-gray-900 dark:text-gray-100 hover:underline">View</Link>
                  <button v-if="memberDividend.status === 'pending' && dividend.status === 'approved'"
                    @click="payMember(memberDividend)"
                    class="text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300">Pay</button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="showApproveModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-900 mx-3 dark:border w-full max-w-md rounded-xl shadow-lg p-5 animate-fadeIn">
        ...
        <div class="flex items-center gap-3 mb-4">
          <div
            class="h-8 w-8 rounded-full bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 flex items-center justify-center font-bold">
            A</div>
          <div>
        ...
        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1" for="approval_notes">Approval Notes
          (optional)</label>
    ...
    <div v-if="showDistributeModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white mx-3 dark:border dark:bg-gray-900 w-full max-w-md rounded-xl shadow-lg p-5 animate-fadeIn">
        ...
        <div
          class="rounded-md bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 p-3 text-sm text-yellow-700 dark:text-yellow-300">
    ...
    <div v-if="showReverseModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white mx-3 dark:border dark:bg-gray-900 w-full max-w-md rounded-xl shadow-lg p-5 animate-fadeIn">
        ...
        <p class="text-sm text-gray-700 dark:text-gray-300 mb-3">
          Are you sure you want to reverse the distribution? This will roll back payments and return funds to the
          system.
        </p>

        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1" for="reversal_reason">Reason for reversal
