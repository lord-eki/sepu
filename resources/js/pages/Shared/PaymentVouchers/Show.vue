<template>
  ...
  <AppLayout :breadcrumbs="[{ title: 'Vouchers', href: '/vouchers' }, { title: `${voucher.voucher_number}` }]">
    <!-- Header -->
    <div class="bg-[#0A2342] text-white rounded-xl px-5 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 shadow">
      <div>
        <h1 class="text-2xl font-semibold">Payment Voucher • {{ voucher.voucher_number }}</h1>
        <p class="text-sm opacity-80 mt-1">
          Created on {{ formatDate(voucher.created_at) }} by <span class="font-medium">{{ voucher.creator?.name || '—' }}</span>
        </p>
      </div>

      <div class="flex items-center gap-3">
        <Link :href="route('vouchers.index')" class="bg-white text-[#0A2342] px-3 py-2 rounded-lg inline-flex items-center gap-2 shadow-sm hover:opacity-95">
        ...
        <div class="relative" ref="actionsRoot">
          ...
          <transition name="fade">
            <div v-if="showActionsMenu" class="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border z-40">
              <div class="py-2">
                <Link v-if="canEdit" :href="route('vouchers.edit', voucher.id)" class="flex items-center gap-2 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">
                  <Pencil class="h-4 w-4" /> Edit Voucher
                </Link>

                <button v-if="voucher.status === 'pending'" @click="submitVoucher" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                  <Paperclip class="h-4 w-4" /> Submit for Approval
                </button>

                <button @click="duplicateVoucher" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                  <Files class="h-4 w-4" /> Duplicate Voucher
                </button>

                <button @click="downloadPDF" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                  <WindArrowDown class="h-4 w-4" /> Download PDF
                </button>

                <button v-if="canDelete" @click="confirmDeleteVoucher" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
    ...
    <div class="max-w-6xl mx-auto space-y-6 pb-12">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div v-if="voucher.loan">
                ...
                <dd class="mt-1 text-slate-800">
                  ...
                  <div class="text-xs text-slate-500">Member: {{ voucher.loan.member?.first_name }} {{ voucher.loan.member?.last_name }}</div>
                </dd>
              </div>
            </div>
          </section>
          ...
          <section v-if="voucher.supporting_documents && voucher.supporting_documents.length" class="bg-white rounded-xl shadow p-5 border">
            <div class="flex items-center justify-between">
              <h3 class="text-md font-semibold text-[#0A2342]">Supporting Documents</h3>

              <div v-if="canEdit && voucher.status === 'pending'">
                <label class="inline-flex items-center gap-2 px-3 py-2 rounded bg-slate-100 text-slate-800 cursor-pointer hover:bg-slate-200">
                  Upload More
                  <input type="file" ref="fileInput" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden" @change="uploadAdditionalDocuments" />
                </label>
              </div>
            </div>

            <div class="mt-4 space-y-3">
              <div v-for="(doc, idx) in voucher.supporting_documents" :key="idx" class="flex items-center justify-between bg-slate-50 p-3 rounded">
                <div class="flex items-center gap-3">
                  <File class="h-6 w-6 text-slate-400" />
                  <div>
                    <p class="font-medium text-slate-800">{{ doc.name }}</p>
                    <p class="text-xs text-slate-500">{{ formatFileSize(doc.size) }} • Uploaded {{ formatDate(doc.uploaded_at) }}</p>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <button v-if="canEdit && voucher.status === 'pending'" @click="confirmDeleteDocument(doc)" class="text-red-600 hover:underline" title="Delete">
                    <Trash class="h-4 w-4" />
                  </button>
                </div>
              </div>
        ...
        <aside class="space-y-6">
          ...
          <div class="bg-white rounded-xl shadow p-4 border">
            ...
            <div class="mt-3 space-y-2">
              <button v-if="canApprove && voucher.status === 'pending'" @click="showApproveModal = true" class="w-full inline-flex items-center gap-2 justify-center px-3 py-2 rounded bg-green-600 text-white hover:brightness-95">
                <CheckCircle class="h-4 w-4" /> Approve
              </button>

              <button v-if="canApprove && voucher.status === 'pending'" @click="showRejectModal = true" class="w-full inline-flex items-center gap-2 justify-center px-3 py-2 rounded bg-red-600 text-white hover:brightness-95">
                <EyeClosed class="h-4 w-4" /> Reject
              </button>

              <button v-if="canPay && voucher.status === 'approved'" @click="showPaymentModal = true" class="w-full inline-flex items-center gap-2 justify-center px-3 py-2 rounded bg-[#0A2342] text-white hover:brightness-95">
                <BanknoteIcon class="h-4 w-4" /> Process Payment
              </button>

              <button @click="duplicateVoucher" class="w-full inline-flex items-center gap-2 justify-center px-3 py-2 rounded bg-slate-600 text-white hover:brightness-95">
                <Files class="h-4 w-4" /> Duplicate
              </button>

              <button @click="downloadPDF" class="w-full inline-flex items-center gap-2 justify-center px-3 py-2 rounded bg-slate-600 text-white hover:brightness-95">
                <WindArrowDown class="h-4 w-4" /> PDF
              </button>

              <button v-if="canDelete" @click="confirmDeleteVoucher" class="w-full inline-flex items-center gap-2 justify-center px-3 py-2 rounded bg-red-600 text-white hover:brightness-95">
            ...
            <div class="mt-3 space-y-4 text-sm text-slate-700">
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500">{{ formatDate(voucher.created_at) }} • {{ voucher.creator?.name || '—' }}</div>
                </div>
              </div>

              <div v-if="voucher.approved_by" class="flex items-start gap-3">
                <div class="flex-shrink-0">
                  <CheckCircle2 v-if="voucher.status === 'approved' || voucher.status === 'paid'" class="h-5 w-5 text-green-500" />
                  <CircleX v-else class="h-5 w-5 text-red-500" />
                </div>
                <div>
                  <div class="font-medium">{{ voucher.status === 'rejected' ? 'Rejected' : 'Approved' }}</div>
                  <div class="text-xs text-slate-500">{{ formatDate(voucher.approval_date) }} • {{ voucher.approver?.name || '—' }}</div>
                  <div v-if="voucher.approval_notes" class="text-xs text-slate-600 mt-1">{{ voucher.approval_notes }}</div>
                </div>
              </div>
              ...
              <div v-if="voucher.status === 'paid'" class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500">{{ formatDate(voucher.payment_date) }} • {{ voucher.payer?.name || '—' }}</div>
                </div>
              </div>
          ...
          <div class="bg-white rounded-xl shadow p-4 border">
            ...
            <div class="mt-3 text-sm text-slate-700 space-y-2">
              <div class="flex justify-between"><span class="text-slate-500">Created</span><span class="font-medium">{{ formatDate(voucher.created_at) }}</span></div>
              <div v-if="voucher.approval_date" class="flex justify-between"><span class="text-slate-500">{{ voucher.status === 'rejected' ? 'Rejected' : 'Approved' }}</span><span class="font-medium">{{ formatDate(voucher.approval_date) }}</span></div>
              <div v-if="voucher.payment_date" class="flex justify-between"><span class="text-slate-500">Paid</span><span class="font-medium">{{ formatDate(voucher.payment_date) }}</span></div>
            </div>
          </div>
        </aside>
      </div>
    ...
    <Modal :show="showApproveModal" @close="showApproveModal = false">
      <div class="p-4">
        ...
        <p class="text-sm text-slate-600 mt-2">Approve payment of <strong>{{ formatCurrency(voucher.amount) }}</strong> to <strong>{{ voucher.payee_name }}</strong>?</p>

        <form @submit.prevent="approveVoucher" class="mt-4 space-y-3">
          <textarea v-model="approvalForm.approval_notes" rows="3" placeholder="Optional approval notes" class="w-full border rounded p-2"></textarea>

          <div class="flex justify-end gap-3">
            <button type="button" @click="showApproveModal = false" class="px-3 py-2 rounded bg-slate-200">Cancel</button>
    ...
    <Modal :show="showRejectModal" @close="showRejectModal = false">
      <div class="p-4">
        ...
        <form @submit.prevent="rejectVoucher" class="mt-4 space-y-3">
          <textarea v-model="rejectionForm.rejection_reason" rows="3" placeholder="Rejection reason (required)" class="w-full border rounded p-2" required></textarea>
          <p v-if="rejectionForm.errors.rejection_reason" class="text-xs text-red-600">{{ rejectionForm.errors.rejection_reason }}</p>

          <div class="flex justify-end gap-3">
            <button type="button" @click="showRejectModal = false" class="px-3 py-2 rounded bg-slate-200">Cancel</button>
    ...
    <Modal :show="showPaymentModal" @close="showPaymentModal = false" max-width="2xl">
      <div class="p-4">
        ...
        <p class="text-sm text-slate-600 mt-2">Process payment to <strong>{{ voucher.payee_name }}</strong> for <strong>{{ formatCurrency(voucher.amount) }}</strong>.</p>
        ...
        <form @submit.prevent="processPayment" class="mt-4 space-y-3">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <input v-model="paymentForm.payment_reference" placeholder="Payment reference (optional)" class="border rounded p-2" />
          </div>

          <textarea v-model="paymentForm.payment_notes" rows="3" placeholder="Payment notes (optional)" class="w-full border rounded p-2"></textarea>

          <div class="mt-3 bg-slate-50 p-3 rounded">
            <div class="flex justify-between text-sm text-slate-700"><div>Payee</div><div class="font-medium">{{ voucher.payee_name }}</div></div>
            <div class="flex justify-between text-sm text-slate-700 mt-1"><div>Amount</div><div class="font-semibold text-[#0A2342]">{{ formatCurrency(voucher.amount) }}</div></div>
          </div>

          <div class="flex justify-end gap-3 mt-3">
            <button type="button" @click="showPaymentModal = false" class="px-3 py-2 rounded bg-slate-200">Cancel</button>
    ...
    <Modal :show="showCancelConfirm" @close="showCancelConfirm = false">
      <div class="p-4">
        ...
        <form @submit.prevent="cancelVoucher" class="mt-4 space-y-3">
          <textarea v-model="cancellationForm.cancellation_reason" rows="3" placeholder="Reason for cancellation" class="w-full border rounded p-2" required></textarea>
          <div class="flex justify-end gap-3">
            <button type="button" @click="showCancelConfirm = false" class="px-3 py-2 rounded bg-slate-200">Close</button>
            <button type="submit" :disabled="cancellationForm.processing" class="px-3 py-2 rounded bg-gray-700 text-white">
    ...
    <Modal :show="showDeleteDocConfirm" @close="showDeleteDocConfirm = false">
      <div class="p-4">
        ...
        <p class="text-sm text-slate-600 mt-2">Are you sure you want to delete <strong>{{ docToDelete?.name }}</strong>?</p>

        <div class="flex justify-end gap-3 mt-4">
          <button @click="showDeleteDocConfirm = false" class="px-3 py-2 rounded bg-slate-200">Cancel</button>
          <button @click="deleteDocumentConfirmed" :disabled="docDeleteProcessing" class="px-3 py-2 rounded bg-red-600 text-white">
    ...
    <Modal :show="showDeleteVoucherConfirm" @close="showDeleteVoucherConfirm = false">
      <div class="p-4">
        ...
        <p class="text-sm text-slate-600 mt-2">This action cannot be undone. Delete voucher <strong>{{ voucher.voucher_number }}</strong>?</p>

        <div class="flex justify-end gap-3 mt-4">
          <button @click="showDeleteVoucherConfirm = false" class="px-3 py-2 rounded bg-slate-200">Cancel</button>
          <button @click="deleteVoucherConfirmed" :disabled="voucherDeleteProcessing" class="px-3 py-2 rounded bg-red-600 text-white">
...
import Modal from '@/components/Modal.vue' // adjust if modal path differs
