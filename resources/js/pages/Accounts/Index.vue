<template>
  ...
  <AppLayout :breadcrumbs="[{ title: 'Accounts', href: '/accounts' }]">
    ...
    <div class="pb-8 max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
      <!-- Header -->
      <h2 class="font-bold text-lg sm:text-xl text-[#0a2342] px-4 pt-4 ">
        Accounts Management
      </h2>
      <!-- Statistics Cards -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div v-for="card in [
    { label: 'Total Accounts', value: stats.total_accounts, color: 'bg-[#0a2342]', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16h14zm-8-4h2v-2h-2v2zm0-4h2v-2h-2v2zm0-4h2V7h-2v2z' },
  ...
  ]" :key="card.label" class="bg-white shadow-md hover:shadow-lg rounded-2xl p-5 flex items-center gap-4 transition">
          <div
            :class="['w-8 sm:w-10 h-8 sm:h-10 flex items-center justify-center rounded-full text-white', card.color]">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div class="relative text-sm">
                <input v-model="filters.search" type="text" placeholder="Search accounts..."
                  class="w-full border border-gray-300 rounded-lg shadow-sm focus:border-[#0a2342] focus:ring-[#0a2342] pl-10 p-2"
            ...
            <div>
              ...
              <select v-model="filters.account_type"
                class="w-full border border-gray-300 text-sm rounded-lg shadow-sm focus:border-[#0a2342] focus:ring-[#0a2342] p-2"
                @change="applyFilters">
            ...
            <div>
              ...
              <select v-model="filters.status"
                class="w-full border border-gray-300 rounded-lg text-sm shadow-sm focus:border-[#0a2342] focus:ring-[#0a2342] p-2"
                @change="applyFilters">
            ...
            <div class="flex items-end gap-2">
              <button @click="clearFilters"
                class="bg-gray-300 hover:cursor-pointer hover:bg-gray-400 text-[#0a2342] px-4 py-2 text-sm rounded-lg font-medium transition">
                Clear
              </button>
              <Link :href="route('accounts.create')"
                class="bg-[#0a2342] hover:bg-blue-800 text-white px-4 py-2 text-sm rounded-lg font-medium transition">
              New Account
              </Link>
            </div>
          </div>
      ...
      <div class="sm:hidden">
        <div v-for="account in accounts.data" :key="account.id"
          ...
          <div class="flex justify-between items-center">
            ...
            <span :class="account.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'"
          ...
          <p class="text-sm font-semibold text-[#0a2342] mt-1">KES {{ formatCurrency(account.balance) }}</p>
          <div class="mt-3 flex gap-2 flex-wrap">
            ...
            <Link :href="route('accounts.withdrawal.show', account.id)" class="text-orange-600 hover:underline">Withdraw
            </Link>
      ...
      <div class="hidden sm:block bg-white text-sm shadow-md rounded-2xl border border-gray-100">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class=" rounded-lg text-[#0a2342] sticky top-0 z-10">
              <tr class="bg-blue-50">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="account in accounts.data" :key="account.id" class="hover:bg-blue-50 transition">
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-sm font-semibold text-[#0a2342]">
                ...
                <td class="px-6 py-4">
                  <div class="flex gap-2">
                    ...
                    Deposit</Link>
                    <Link :href="route('accounts.withdrawal.show', account.id)" class="text-red-800 hover:underline">
                    Withdraw</Link>
                    <Link :href="route('accounts.edit', account.id)" class="text-gray-600 hover:underline">Edit</Link>
                  </div>
                </td>
