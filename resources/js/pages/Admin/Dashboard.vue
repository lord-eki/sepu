<template>
  <AppLayout :breadcrumbs="[{ title: 'Dashboard', href: '/dashboard' }]">
    ...
    <div class="min-h-screen bg-slate-50 dark:bg-[#081122]">
      <!-- TOP BAR -->
      <div
        class="relative overflow-hidden border-b border-white/10 bg-gradient-to-br from-blue-900 via-blue-950 to-slate-900 px-6 py-6 sm:px-8 rounded-b-3xl shadow-2xl shadow-blue-950/20">
        <div
          class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(249,115,22,0.18),_transparent_30%)]" />
        <div class="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <div
              class="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/10 text-sm font-semibold text-white shadow-lg backdrop-blur-xl">
              A
            </div>
          </div>
        </div>
      ...
      <div class="px-6 py-8 sm:px-8">
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <div v-for="stat in quickStats" :key="stat.title"
            ...
            <div
              class="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-slate-50/40 dark:to-slate-800/20" />
            ...
            <div class="relative flex items-start justify-between">
              ...
              <div
                class="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-500 to-orange-400 text-white shadow-lg shadow-orange-500/20">
      ...
      <div class="grid grid-cols-1 gap-6 px-6 pb-8 sm:px-8 xl:grid-cols-3">
        ...
        <div class="space-y-6 xl:col-span-2">
          <!-- Pending Approvals -->
          <div
            class="rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          ...
          <div
            ...
            <div class="custom-scroll max-h-[460px] space-y-3 overflow-y-auto pr-1">
              <div v-for="(item, i) in recentActivities" :key="i" @click="item.link ? $inertia.visit(item.link) : null"
                class="cursor-pointer rounded-2xl border px-4 py-3 transition-all duration-300 hover:shadow-md" :class="[
    item.status === 'pending'
                ...
                <div class="flex items-start justify-between gap-4">
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-400">
                      ...
                      <span v-if="item.status" class="rounded-full px-2 py-0.5 text-xs font-semibold capitalize" :class="[
    item.status === 'pending'
      ...
      : item.status === 'approved'
        ...
        : item.status === 'rejected'
          ...
          : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'
                      ...
                      <span v-if="item.status === 'pending'" class="font-medium text-orange-500">
                        Click to view →
                      </span>
                    </div>
                  </div>
          ...
          <div
            ...
            <div class="space-y-3">
              <div v-for="setup in setupItems" :key="setup.title" @click="$inertia.visit(setup.link)"
                ...
                <div
                  class="flex h-10 w-10 items-center justify-center rounded-2xl bg-orange-100 text-orange-500 dark:bg-orange-500/10 dark:text-orange-300">
      ...
      <!-- Footer -->
      <div
        class="border-t border-slate-200 px-6 py-5 text-center text-xs text-slate-400 dark:border-slate-800 dark:text-slate-500 sm:px-8">
