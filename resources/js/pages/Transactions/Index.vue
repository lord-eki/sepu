<template>
  <AppLayout :breadcrumbs="[{ title: 'Transactions', href: route('transactions.index') }]">
    ...
    <!-- Page Header -->
    <div class="bg-white border-l-4 border-orange-500 rounded-xl shadow-sm p-5 mx-5 mt-6 flex flex-col md:flex-row md:items-center md:justify-between">
      <h1 class="text-2xl font-bold text-[#0a2342]">Transactions</h1>
      ...
      <div class="mt-4 md:mt-0 flex flex-wrap gap-2">
        <input
          ...
          class="border border-gray-300 focus:ring-2 focus:ring-orange-500 rounded-full px-4 py-2 w-60 text-sm text-gray-700 transition-all duration-200"
        />

        <select
          v-model="filters.transaction_type"
          class="border border-gray-300 focus:ring-2 focus:ring-orange-500 rounded-full px-4 py-2 text-sm text-gray-700 transition-all duration-200"
        >
        ...
        <button
          @click="applyFilters"
          class="bg-orange-500 hover:bg-orange-600 transition-colors text-white text-sm rounded-full px-5 py-2 font-medium shadow"
        >
          Apply
        </button>
      </div>
    ...
    <!-- Statistics Cards -->
    <div class="grid md:grid-cols-5 sm:grid-cols-2 gap-4 mb-8 mt-8 px-5">
      <div
        v-for="(value, key) in statistics"
        :key="key"
        class="bg-white shadow-md hover:shadow-lg transition-all duration-300 rounded-2xl p-4 border-0.5"
        :class="{
    ...
    <div class="overflow-x-auto bg-white rounded-2xl mx-5 shadow border border-blue-100">
      <table class="min-w-full divide-y divide-blue-100">
        <thead class="bg-gradient-to-r from-[#0a2342] to-[#1d4ed8] text-white">
          <tr>
    ...
    <div class="flex justify-between items-center mt-8 mb-10 text-sm text-gray-700 px-5">
      ...
      <div class="flex gap-2">
        <button
          ...
          class="px-4 py-1.5 rounded-full border border-blue-700 text-blue-800 hover:bg-blue-900 hover:text-white transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Prev
        </button>
        <button
