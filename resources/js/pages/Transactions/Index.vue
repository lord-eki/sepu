<template>
  <AppLayout :breadcrumbs="[{ title: 'Transactions', href: '/transactions' }]">
    ...
    <div class="space-y-8 bg-[#F4F6F8] min-h-screen p-2 sm:p-4">
      ...
      <div class="bg-white rounded-2xl p-6 shadow space-y-4 border border-gray-100">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <select v-model="filters.status" class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#0A2342]">
        ...
        <div class="flex flex-wrap gap-3 pt-2">
          ...
          <button @click="resetFilters" class="px-5 py-2.5 border rounded-lg hover:bg-gray-50 transition">
            Reset
          </button>

          <button @click="" class="px-5 py-2.5 bg-orange-300 border rounded-lg hover:bg-orange-400 transition">
            Export
          </button>
      ...
      <div class="bg-white rounded-2xl p-6 shadow border border-gray-100 overflow-x-auto w-full">
        <table class="min-w-max divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="(t, idx) in pageData" :key="t.id" class="hover:bg-[#0A2342]/5 transition">
    ...
    statusBadge(t.status)
      ...
      <!-- APPROVE -->
      <div v-if="modals.approve" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      <!-- REJECT -->
      <div v-if="modals.reject" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-lg">
      ...
      <!-- REVERSE -->
      <div v-if="modals.reverse" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-lg">
...
async function fetchTransactions(page = 1) {
  try {
    ...
    if (data) {
      transactions.value = {
        data: data.data,       // transaction rows
