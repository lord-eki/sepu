  ...
  <AppLayout>
    ...
    <div class="py-16 mx-2 flex flex-col items-center text-center space-y-6">
      ...
      <div class="bg-yellow-100 dark:bg-yellow-900 p-4 sm:p-6 rounded-full">
        <svg class="w-12 sm:w-16 h-12 sm:h-16 text-yellow-500 dark:text-yellow-300" fill="none" stroke="currentColor"
      ...
      <!-- Title -->
      <h2 class="max-sm:px-6 text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-100">
        Your Profile is Awaiting Approval
      </h2>

      <!-- Message -->
      <p class="max-sm:text-sm max-w-xl text-gray-600 dark:text-gray-400">
      ...
      <!-- Action button -->
      <div class="pt-6">
        <Link :href="route('logout')" method="post" as="button" class="px-5 py-2 hover:cursor-pointer rounded-xl bg-blue-800 text-white hover:bg-blue-700 
    ...
    <div>
      ...
      <transition name="fade" class="max-sm:px-2">
        <div v-if="showPaymentModal" class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
